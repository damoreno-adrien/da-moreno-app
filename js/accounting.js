...
function fetchAllBranches() {
    onSnapshot(collection(db, "branches"), snap => {
        ...
        } else { currentBranchFilter = currentUserAccessibleBranches[0] || 'ALL'; }
        renderPaymentsList();
    });
}
...
function fetchReceivedOrders() {
    const q = query(collection(db, "purchase_orders"), where("status", "==", "Received"));
    onSnapshot(q, snap => {
...
function renderPaymentsList() {
    if (allBranches.length === 0 || allSuppliers.length === 0) return;
    ...
    filteredOrders.sort((a,b) => (a.financials?.paymentDueDate || '9999-12-31').localeCompare(b.financials?.paymentDueDate || '9999-12-31'));
...
function openPayModal(orderId) {
    ...
    document.getElementById('pay-amount-display').textContent = `${financials.totalAmount ? financials.totalAmount.toLocaleString('en-US', { minimumFractionDigits: 2 }) : '0.00'} THB`;
...
async function confirmPayment() {
    ...
    if (!payDateInput.checkValidity()) { payDateInput.reportValidity(); return; }
    ...
    try {
        ...
        // 1. Maj du PO Parent
        batch.update(doc(db, "purchase_orders", orderToPay.id), { status: "Paid", financials: financials });
        
        // 2. Maj des Enfants
        const childQ = query(collection(db, "orders"), where("poId", "==", orderToPay.id));
        const childSnap = await getDocs(childQ);
        childSnap.forEach(childDoc => batch.update(childDoc.ref, { status: "Paid" }));

        await batch.commit();
        showToast("Order marked as Paid!");
        document.getElementById('pay-modal').classList.replace('flex', 'hidden');
    } catch (e) { showToast("Error processing payment", true); }
}

function setupEventListeners() {
    document.getElementById('accounting-branch-select')?.addEventListener('change', (e) => { currentBranchFilter = e.target.value; renderPaymentsList(); });
    document.getElementById('payments-list').addEventListener('click', (e) => { if (e.target.classList.contains('open-pay-btn')) openPayModal(e.target.dataset.id); });
    document.getElementById('confirm-payment-btn').addEventListener('click', confirmPayment);
    document.querySelectorAll('.close-modal').forEach(btn => btn.addEventListener('click', (e) => { e.target.closest('.fixed.inset-0').classList.replace('flex', 'hidden'); }));
}
